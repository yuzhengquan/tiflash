...
void Block::checkNumberOfRows() const
{
    ...
    {
        if (!elem.column)
            throw Exception("Column " + elem.name + " in block is nullptr, in method checkNumberOfRows.", ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH);
...
Block Block::copyWithColumns(const MutableColumns & columns) const
{
    ...
    size_t num_columns = data.size();
    for (size_t i = 0; i < num_columns; ++i)
        res.insert({nullptr, data[i].type, data[i].name, data[i].column_id});
